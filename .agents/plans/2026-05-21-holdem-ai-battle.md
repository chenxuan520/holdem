...
- 2026-06-03 16:15：新增「AI 擂台 + 排行榜」（前端「擂台排行」标签 / `ArenaView`）。一个编排器自动批量开**全自动观战桌**跑循环联赛，再聚合成可排序排行榜（头牌夺冠率 + 多人 Elo 评分 + 平均名次 / bb100 / 净筹码 / 出错率）。设计要点：
  - **复用现有全自动观战引擎，不改扑克规则核心**：`{spectatorMode:true,semiAutoMode:false,manualMode:false}` 本来就能无人值守跑到结束（Go 每桌一个 goroutine、CF 每桌一条 `setAlarm` 链）；编排器只负责建桌→轮询→聚合。
  - **赛制默认「全员同桌」**：`tableSize=0`→`min(模型数,6)`，模型 ≤ 桌容量时每轮全员同坐一桌、重复 `rounds` 局（零配对偏差 + 真多人博弈，绕开 1v1 和"同桌对手强弱"两个坑）；模型 > 桌容量才均衡切桌；`tableSize=2` 作为可选 heads-up 基准。
  - **算分走共享 WASM 核心保证双后端一致**：`backend/internal/match/tournament.go` 新增纯函数 `CoreBuildSchedule`（确定性赛程）+ `CoreAggregateStandings`（名次 = 最终筹码降序、并列淘汰更晚者更高；按 preset 聚合各项 + 多人 Elo 按对局结束时间顺序两两展开更新 + 取自 AILogs 的可靠性）。在 `backend/cmd/wasmcore/main.go` 注册 `holdemBuildSchedule` / `holdemAggregateStandings`，`cf/src/core.ts` 加类型包装，跑 `cd cf && npm run build:wasm` 重生成 `core.wasm`。
  - **Go 后端**：`backend/internal/match/orchestrator.go` 新增 `Service.tournaments` 单例 + worker-pool goroutine（限 `maxConcurrency`，建桌、轮询 `GetMatch` 到终态、超 `maxHandsPerMatch` 用 `control:"stop"` 收尾、卡住检测、`CoreAggregateStandings` 聚合）；`store/sqlite.go` 加 `tournaments` 表（payload_json，通过结构化匹配的可选 `tournamentStore` 接口持久化，不动 `ReplayStore` 故不破坏测试假实现）；启动把 `running` 标 `interrupted`（不自动续跑）；`httpapi/server.go` 加 `POST/GET /api/tournaments`、`GET/POST(control stop)/DELETE /api/tournaments/{id}`。
  - **CF 后端**：新增单例 `TournamentDO`（`getByName("arena")`，自带 SQLite + `POLL_MS` alarm 轮询循环）：RPC 调 `MatchDO`/`RegistryDO`，用 `RegistryDO.getReplay` 非空判完成，超手数 `control("stop")`，`core.aggregateStandings` 聚合。给 `MatchDO` 加了一个非阻塞 `createAuto`（持久化后挂自身 alarm 自驱，避免 arena alarm 卡在一个完整 ~40-fetch chunk 上）。`wrangler.jsonc` 加 binding + 新 migration `v2`（不改 v1），`env.ts` 补类型，`index.ts` 加 `/api/tournaments` 路由。
  - **成本闸门**（与"纯 AI 观战默认半自动"原则调和：联赛是显式开启、有界、可随时停）：两后端 `normalize*Config` 硬夹 `maxConcurrency≤6 / maxHandsPerMatch≤1000 / maxMatches≤500`；停止开关停掉新建 + 在跑桌；前端开跑前显示"预计 N 桌 / 最多约 D 次决策"估算，含外部 endpoint 时弹付费警告，推荐 CF + Workers-AI 免费模型。上榜池 v1 仅限内置 preset（稳定身份），inline 自定义模型后续再支持。
  - **测试**：`tournament_test.go` 覆盖赛程（全员同桌 / 均衡切桌 / MaxMatches 截断 / <2 preset 报错）+ 聚合（夺冠率/平均名次/bb100/净筹码/可靠性 + Elo 赢家>1500>输家 + 三人桌名次 A>C>B）；`orchestrator_test.go` 用本地 fake AI server（恒 all_in，几手即淘汰，无重试 backoff）跑端到端联赛到 `finished`（0.4s）+ 停止流程 + 校验。验证：`cd backend && go test ./...` 全过、`GOOS=js GOARCH=wasm go build ./cmd/wasmcore` 通过、`cd cf && npm run typecheck` + `wrangler deploy --dry-run`（三个 DO + AI binding 正常）、`cd frontend && npm run build && npm run test`（38 个全过）。剩余：CF 真机 smoke（`wrangler dev` 会发真实 Workers-AI 调用）留给用户；inline 自定义模型上榜、duplicate poker、token 成本列为后续迭代。
- 2026-06-09 17:45：补通了本机 `ttadk` 的 `glm-5.1` 兼容路径。排查发现 `ttadk` 注入给 opencode 的 provider config 里，`glm-5.1 / glm-5 / kimi-k2.5 / deepseek-v4-pro` 实际都挂在 `@ai-sdk/anthropic` 风格的 llmbox provider 下；直接裸打现有后端的 `/chat/completions + Authorization` 会把 `glm-5.1` 误打成 `403 model not allowed`，而补上 `X-Source: ttadk` 后可正常命中模型，只是其返回的 `message.content` 不是纯字符串而是 `[{type:"thinking"}, {type:"text"}]` 的 block 数组。为此后端做了三处最小兼容：1）`backend/internal/config/presets.go` 新增私有 `extra_headers` 字段，允许本机 preset 为特定网关附加静态 header；2）`backend/internal/ai/{parse.go,client.go,export.go}` 新增 array-content 解析，只提取 `type=text` block、忽略 `thinking`，并让请求/探活都合并 `extra_headers`；3）probe 文案改成更收敛的 `Reply with exactly pong.`，且对高 `max_tokens` preset 把探活预算上调到最多 `128`，避免 `glm-5.1` 这种 reasoning 模型把 16 token 全耗在 thinking 上。对应本机 `config/ai-presets.yaml` 里的 `glm-5-1` 现已加上 `extra_headers: {X-Source: ttadk}`、`extra_body: {reasoning_effort: none}` 和 `max_tokens: 512`。新增/更新测试：`backend/internal/ai/client_test.go` 验证 extra header 会真正发出且 array content 能解析；`parse_test.go` 锁住“忽略 thinking block，只保留 text”；`client_payload_test.go` 锁住新的 probe payload 规则。联调结论：`glm-5.1` 现在能在 `json_object + X-Source: ttadk + reasoning_effort:none` 路径上返回带 text block 的 JSON 内容；本地 live probe 已在后端重启后返回 `ok:true, snippet:"pong"`。当前风险：`glm-5.1` 仍是明显的 reasoning-heavy 模型，简短探活或太小的 `max_tokens` 很容易只拿到 thinking block；本轮通过 header、`reasoning_effort:none`、更高 `max_tokens` 和 block 解析兼容把功能链路打通，但它的单位调用成本/时延仍会高于 `glm-5`。
- 2026-06-09 18:10：按用户给出的 Volcano Engine Ark OpenCode provider 配置，把一批可直连的 Ark 模型加入了本机私有 `config/ai-presets.yaml`，并逐个做了结构化输出兼容性筛选。筛选方法不是只测“能不能 pong”，而是按本项目真实使用形态直接打 `chat/completions`：`deepseek-*` 优先验证 `tool_call`，其余模型优先验证 `json_object`；对首轮失败的模型再补测 `tool_call` / 纯 JSON 提示词 fallback。最终确认可用并已加入的 Ark preset 有 11 个：`ark-ark-code-latest`、`ark-doubao-seed-code`、`ark-glm-5-1`、`ark-deepseek-v4-flash`、`ark-deepseek-v4-pro`、`ark-doubao-seed-2-0-code`、`ark-doubao-seed-2-0-pro`、`ark-doubao-seed-2-0-lite`、`ark-minimax-m2-7`、`ark-minimax-m3`、`ark-kimi-k2-6`。配置决策：`deepseek-v4-flash / deepseek-v4-pro` 走 `tool_call + thinking: disabled`；`glm-5.1` 走 `tool_call + reasoning_effort: none`；`doubao-seed-2.0-pro / doubao-seed-2.0-lite / minimax-m3 / kimi-k2.6` 走 `json_object`；`ark-code-latest / doubao-seed-code / doubao-seed-2.0-code / minimax-m2.7` 走 `tool_call`。全部条目都加了 `ark-` 前缀，避免与已有 llmbox preset id 冲突。完成后重启后端并通过 `GET /api/presets` 确认全部加载；随后逐个执行 `POST /api/presets/{id}/probe`，11 个新增 Ark preset 全部返回 `ok: true`。额外发现：`ark-ark-code-latest` 的 probe 虽然成功，但返回体里的 `model` 字段显示为 `deepseek-v4-pro`，像是 Ark 侧的 alias / 路由映射异常，因此它目前可以用，但如果后续拿它做严肃 benchmark，需要把这个 provider 侧身份不透明风险单独记在心里。
- 2026-06-09 18:24：继续排查用户反馈的 `MiniMax-M3 @Ark` / `KIMI-K2.6 @Ark`“卡住像死掉”。问题不在前端，而在 Ark 这两条模型默认会在真实德州 prompt 下长篇输出 `reasoning_content`，导致 `finish_reason=length`、`content` 为空，表现就像一手一直不落动作。做了定点参数实验：`reasoning_effort:none` 对两者都**无效**，`enable_thinking:false` 也无效；但和 Ark DeepSeek 一样，`extra_body: { thinking: { type: disabled } }` 对两者都有效。实测在真实牌桌 prompt + `max_tokens: 192` 下，`MiniMax-M3` 和 `KIMI-K2.6` 都变成 `finish_reason=stop`、`reasoning_len=0`，并直接输出合法 JSON 动作。因此把本机 `config/ai-presets.yaml` 里的 `ark-minimax-m3` / `ark-kimi-k2-6` 都补上了 `thinking: disabled`，重启后端后重新 probe 通过；再各做一把 2 人手动观战 live smoke：`ark-minimax-m3` 一步就落出 preflop `fold`，`ark-kimi-k2-6` 一步落出 preflop `raise 6`，都不再出现“长考到自动 fold”的现象。结论：这两个 Ark 模型可以保留，但必须带 `thinking: disabled`；否则在本项目的德州 prompt 下基本不可用。
- 2026-06-09 18:31：用户进一步要求“Ark 都给我关了”，于是把全部 Ark preset 统一改成 `extra_body: { thinking: { type: disabled } }`，包括此前只配了 `reasoning_effort:none` 的 `ark-glm-5-1` 也一并切过去。切换前做了全量直连兼容性检查：11 个 Ark 模型在各自当前的结构化模式下都能接受这个参数；其中绝大多数会把 `reasoning_content` 降到 0，`ark-minimax-m2-7` 仍会带一小段 reasoning 但会按时返回 tool_call，不再卡住。随后重启后端，并对 11 个 Ark preset 全量执行 `POST /api/presets/{id}/probe`，全部 `ok: true`；`ark-glm-5-1` probe 延迟降到约 1.3s，`ark-minimax-m3` / `ark-kimi-k2-6` 也都在秒级完成。当前剩余注意点：`ark-kimi-k2-6` 在直连 `json_object` 测试里偶尔会把 JSON 包进 markdown fence，但本项目 `parse.go` 已能剥掉 fence；`ark-ark-code-latest` 仍存在 provider 回包 `model` 显示成 `deepseek-v4-pro` 的身份不透明问题，这和 thinking 开关无关，后续若做严肃 benchmark 仍需谨慎对待。
- 2026-08-07：新增服务端擂台命令行入口 `backend/cmd/arena`，无需网页即可通过现有 `/api/tournaments` 创建联赛、轮询进度并在终端输出排行榜。命令默认连接本地 `:18130`，不传 preset id 时读取 `/api/presets` 选择全部内置模型，也支持位置参数指定参赛池、`--api`/`HOLDEM_API_URL` 切换 Go 或 CF 后端、`HOLDEM_AUTH_PASSWORD` 鉴权，以及轮数、桌型、手数、并发、总场次、seed、异步创建等参数。实现只做 HTTP 客户端，不复制赛事引擎或直写 SQLite，因此继续复用两后端现有编排、持久化和成本上限；CLI 额外拒绝会制造无界排赛或无效牌桌的参数与重复 preset、拒绝携带密码跟随 HTTP 重定向，并在等待模式被中断或轮询失败时主动停止赛事，避免无人值守继续烧 token。为缩小“服务端已创建但响应 ID 前连接中断”的孤儿赛事窗口，创建请求开始后不再被 Ctrl-C 半途取消（仍受 30 秒 HTTP timeout），取得 ID 后立即按中断语义停止（等待与 `--wait=false` 两种模式一致）；若仍无法确认创建结果则明确提示先检查赛事列表、不要盲目重试。新增 `backend/cmd/arena/main_test.go` 覆盖自动选择全部 preset + 鉴权、等待完成并打印榜单、后端错误透传、参数边界、重定向防泄漏、自动发现重复 preset、两种模式创建期间取消后的停止、轮询失败自动停止和失败场次非零结果；README 补充命令示例、退出语义和真实 AI 费用提示。
- 2026-08-07：扩充本机私有 `config/ai-presets.yaml` 的 llmbox 模型池（该文件仍 gitignored、不会提交 token）。TTADK 新增并实测 `kimi-k3`、`glm-5.2`、`deepseek-v4-flash-0731`、`kimi-k2.5`；NADK 独立接入 `gpt-5.5`（preset `gpt-5-5-nadk`，`https://llmbox.bytedance.net/v1`）。用户明确不要 `gpt-5.6-sol`，且当前账号 probe 也返回 model-not-allowed 403，因此未保留。兼容参数经真实项目形态测试确定：GPT-5.5 走 `tool_call`、删除默认 `temperature`、带 `X-Source: Novel-AI4SE`；Kimi-K3 / GLM-5.2 走 `json_object + thinking.disabled`；Kimi-K2.5 走 `json_object + reasoning_effort:none`；DeepSeek-Flash-0731 走 `tool_call + thinking.disabled`；旧 `gpt-5.3-codex` / `deepseek-v4-pro` 补 `X-Source: ttadk` 后从 403 恢复。全量 llmbox/NADK probe 最终通过，新模型另用真实 `submit_action`/JSON 请求验证合法动作输出，而非只测 pong。NADK token 与 TTADK 不同，为防 TTADK 全量刷新误覆盖，GPT-5.5 token 在 YAML 中故意加引号，并新增本机 `~/.local/bin/holdem-apply-nadk-token`；维护命令与原因已写入 `AGENTS.md`。
- 2026-08-07：对新增模型之前已有的 19 个本机 preset 做全量最小 probe。18 个直接通过；唯一失败的 `ark-glm-5-1` 在 Coding Plan endpoint 返回 `UnsupportedModel`，改测 Ark 标准 `/api/v3` 仍返回 `InvalidEndpointOrModel.NotFound`，确认是上游已下线/当前凭证无权访问，而非本项目参数错误。由于已有可用的 llmbox `glm-5-1`，从私有 YAML 删除这个重复且必然自动 fold 的坏 preset。重启后端后 `/api/presets` 为 23 个且不再包含坏条目，`go test ./...` 全过，浏览器刷新后显示 `23 内置 / 0 自定义`、无 console error；前后端按用户要求继续运行供试玩。
- 2026-08-07：根据用户实战反馈纠正 Kimi-K3 的“可用”判断。简单 pong probe 和短 JSON 测试虽通过，但真实 6 人桌日志显示它连续 3 次把英文推理直接写入 `message.content`，每次恰好耗尽 `max_tokens=512`、`finish_reason=length`，最终没有 JSON，触发请求出错自动 fold；牌桌并非状态机死锁，而是三次长请求结束后停在正常 `hand_complete`。定点测试确认 `thinking:{type:disabled}`、`reasoning_effort:none`、顶层 `enable_thinking:false`、`chat_template_kwargs.enable_thinking:false`、`thinking_type:disabled`、普通/强制 tool call 均无法关闭该路由的强制推理。只有把上限增至 2048 才会在约 43 秒、1590 completion tokens 后产出 JSON，已贴近后端 45 秒单请求 timeout，成本和延迟均不适合扑克多轮决策。因此终止并保留现场桌 `14c1f356074c11be` 的记录，从私有 YAML 移除 `kimi-k3`，重启后端后剩余 22 个内置 preset；页面已刷新且无 console error，`go test ./...` 通过。此前实现日志中“Kimi-K3 结构化测试通过”只适用于过短测试 prompt，不代表实战可用，本条为最终结论。

## 审查

- 2026-08-07：审查 `backend/cmd/arena/{main.go,main_test.go}` 与 README 命令行擂台说明（对照本 plan，基线 `origin/master`/`bfe1167`）。结论：不通过，需 builder 处理。阻塞项：`--rounds` 无上限且服务端在应用 `maxMatches` 前展开完整赛程，可触发巨量分配；等待时 Ctrl-C 或一次轮询失败只退出客户端、不停止仍在产生费用的赛事，CLI 也没有无浏览器 stop 路径；`stopped/interrupted/failed` 及包含失败子赛事的结果均按成功返回，且 `stopped` 状态可能尚未排空在跑桌；默认 HTTP 重定向会把自定义 `X-Holdem-Password` 转发到跨主机目标；筹码/盲注参数缺少关系校验，会创建随后全部子赛事失败的联赛。需补覆盖超大 rounds、取消/轮询故障后的停止语义、停止排空、失败退出码、跨主机重定向鉴权头、非法筹码/盲注组合的测试。已参考用户提供的 `cd backend && go test ./...` 与 `go run ./cmd/arena --help` 通过记录；未改业务代码。
- 2026-08-07 复审：上述参数边界、轮询/等待失败停止、终态错误、重定向鉴权和筹码/盲注问题已修复；`go test ./cmd/arena` 与 scoped `git diff --check` 通过。结论仍不通过：创建赛事的 POST 若在服务端已启动赛事后发生超时、取消或响应解码失败，CLI 尚未取得 ID，当前直接返回且无法停止，仍可能留下无人值守的付费赛事；另外重复 preset 校验只覆盖位置参数，默认从 `/api/presets` 自动装载的 ID 在追加后没有再校验，CF 的自定义 `HOLDEM_PRESETS` 可返回重复 ID 并绕过保护。需 builder 继续处理这两项并补相应测试。
- 2026-08-07 最终复审：自动发现 preset 的重复校验、创建期间保留请求直到取得 ID、超时结果未知提示及取消期间取得 ID 后停止的测试均已补齐；`go test ./cmd/arena` 与 scoped `git diff --check` 通过。结论仍不通过：创建请求使用 `context.WithoutCancel` 后，若调用者传了 `--wait=false`，创建期间收到 Ctrl-C/SIGTERM 仍会在取得 ID 后先命中异步返回分支并成功退出，不会执行停止；这会把明确取消的赛事留在后端继续产生费用。需在 `--wait=false` 返回前优先处理已取消的父 context，并补异步创建期间取消的回归测试。
- 2026-08-07 最终复审通过：`--wait=false` 现会在不可中断的创建响应返回后优先检查父 context，并在创建期间发生取消时使用已取得的赛事 ID 执行停止；回归测试已表格化覆盖默认等待与 `--wait=false` 两条路径。复核 `backend/cmd/arena/{main.go,main_test.go}` 及 README，对照本 plan 与 `origin/master` 基线未发现剩余明确问题；`go test ./cmd/arena` 和 scoped `git diff --check` 通过。残余风险仅为 README 已明确记录的创建请求超时后结果未知，这是现有非幂等 API 下无法由客户端彻底消除的边界。
- 2026-08-10 提交前审查通过：基于本 plan 与 `origin/master`/`bfe1167`，复核预定提交中的 arena CLI、AI provider 兼容改动、配置脱敏边界及 README/AGENTS；明确排除 `config/app.json`、`frontend/tsconfig.app.tsbuildinfo` 和非预定的 `backend/wasmcore`。未发现明确 bug、行为回归、凭证泄漏或无界 AI 成本路径。验证：`go test -count=1 ./cmd/arena ./internal/ai ./internal/config`、`go test ./...`、`go vet ./cmd/arena ./internal/ai ./internal/config`、`go run ./cmd/arena --help`、scoped `git diff --check`、CF WASM 重建及 `npm run typecheck` 均通过；`cf npm test` 在构建 WASM 后因仓库当前没有 Vitest 文件而退出 1，并非本次改动的测试失败。残余风险：未发起会产生真实费用的 provider/擂台 live smoke；创建响应超时后的结果未知边界已在 README 明示。
